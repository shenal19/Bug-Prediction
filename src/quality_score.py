...
def compute_quality_details(prob_bug: np.ndarray, X: pd.DataFrame, metric_weights=None):
    """Explainable quality scoring per sample.
    Returns a list[dict] with fields:
      - score (0..100)
      - band (Excellent/Good/Fair/Poor/Critical)
      - total_risk, model_risk, structural_risk
      - penalties: {loc, complexity, coupling}
      - contributions: {model, loc, complexity, coupling}
      - dominant_factor: one of ['model', 'loc', 'complexity', 'coupling']
    """
    if metric_weights is None:
        metric_weights = {'loc': 0.25, 'complexity': 0.45, 'coupling': 0.30}
    ...
    # Build normalized penalties (Series or scalar fallback)
    loc_pen = safe_norm(X[col_loc_candidates[0]]) if col_loc_candidates else pd.Series(0.5, index=X.index)
    cmp_pen = safe_norm(X[col_complexity_candidates[0]]) if col_complexity_candidates else pd.Series(0.5, index=X.index)
    cpl_pen = safe_norm(X[col_coupling_candidates[0]]) if col_coupling_candidates else pd.Series(0.5, index=X.index)

    model_risk = np.clip(np.asarray(prob_bug).reshape(-1), 0, 1)
    structural_risk = (
        metric_weights['loc']*loc_pen.values +
        metric_weights['complexity']*cmp_pen.values +
        metric_weights['coupling']*cpl_pen.values
    )
    total_risk = np.clip(0.7*model_risk + 0.3*structural_risk, 0, 1)
    score = 100.0 * (1.0 - total_risk)

    contrib_model = 0.7 * model_risk
    contrib_loc = 0.3 * metric_weights['loc'] * loc_pen.values
    contrib_cmp = 0.3 * metric_weights['complexity'] * cmp_pen.values
    contrib_cpl = 0.3 * metric_weights['coupling'] * cpl_pen.values
    ...
    for i in range(len(score)):
        contributions = {
            'model': float(contrib_model[i]),
            'loc': float(contrib_loc[i]),
            'complexity': float(contrib_cmp[i]),
            'coupling': float(contrib_cpl[i])
        }
        dominant_factor = max(contributions, key=contributions.get)
        out.append({
            'score': float(score[i]),
            'band': band_from_score(score[i]),
            'total_risk': float(total_risk[i]),
            'model_risk': float(model_risk[i]),
            'structural_risk': float(structural_risk[i]),
            'penalties': {
                'loc': float(loc_pen.values[i]),
                'complexity': float(cmp_pen.values[i]),
                'coupling': float(cpl_pen.values[i])
            },
